...
export const patientApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    getTodaysMedicines: builder.query({
      query: () => '/prescriptions/today',
      providesTags: [{ type: 'Prescriptions', id: 'TODAY' }],
      transformResponse: (response) => response.data,
    }),

    markMedicineTaken: builder.mutation({
      query: ({ prescriptionId, medicineId, timeIndex }) => ({
        url: `/prescriptions/${prescriptionId}/medicines/${medicineId}/taken`,
        method: 'POST',
        body: { timeIndex },
      }),
      invalidatesTags: [
