...
import { useSearchMedicinesQuery } from "../../../../src/store/api/medicineApi";

const medicineSchema = z.object({
  name: z.string().min(1, 'Medicine name is required'),
  dosage: z.string().min(1, 'Dosage is required'),
  form: z.enum(['tablet', 'capsule', 'liquid', 'injection', 'topical', 'inhaler']),
...
export default function CreatePrescription() {
  const { patientId, patientName, patientAge, patientGender } = useLocalSearchParams();
  ...
  const { data: medicineSuggestions } = useSearchMedicinesQuery(
    { query: medicineSearch, limit: 5 },
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      Alert.alert('Error', error.data?.message || 'Failed to create prescription');
    } finally {
      setLoading(false);
    }
  };
    ...
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.form}>
        ...
        <View style={styles.section}>
          ...
          <View style={[styles.inputContainer, errors.diagnosis && styles.inputError]}>
            <Icon name="stethoscope" size={20} color="#95a5a6" style={styles.inputIcon} />
            <Controller
              control={control}
              name="diagnosis"
          ...
          {fields.map((field, index) => (
            <View key={field.id} style={styles.medicineCard}>
              ...
              <View style={styles.fieldContainer}>
                ...
                <View style={[styles.inputContainer, errors.medicines?.[index]?.name && styles.inputError]}>
                ...
                {errors.medicines?.[index]?.name && (
                  <Text style={styles.errorText}>{errors.medicines[index].name.message}</Text>
                )}

                {/* Medicine Suggestions */}
                {showMedicineSuggestions && medicineSearch.length >= 2 && medicineSuggestions?.data && (
                    ...
                    {medicineSuggestions.data.map((med, idx) => (
                      <TouchableOpacity
                        ...
                        onPress={() => {
                          ...
                          if (med.strength) {
                            setValue(`medicines.${index}.dosage`, med.strength.value + med.strength.unit);
                          }
              ...
              <View style={styles.rowContainer}>
                <View style={[styles.fieldContainer, styles.halfField]}>
                  ...
                  <View style={[styles.inputContainer, errors.medicines?.[index]?.dosage && styles.inputError]}>
                    <Controller
                      control={control}
                  ...
                  {errors.medicines?.[index]?.dosage && (
                    <Text style={styles.errorText}>{errors.medicines[index].dosage.message}</Text>
                  )}
                </View>
                      ...
                      render={({ field: { onChange, value } }) => (
                        ...
                        >
                          {compartments.map(comp => (
                            <Picker.Item key={comp.value} label={comp.label} value={comp.value} />
                          ))}
                        </Picker>
                      )}
                    />
                ...
                {watch(`medicines.${index}.times`)?.map((time, timeIndex) => (
                  <View key={timeIndex} style={styles.timeRow}>
                    ...
                    {watch(`medicines.${index}.times`).length > 1 && (
                      <TouchableOpacity onPress={() => removeTimeFromMedicine(index, timeIndex)}>
              ...
              <View style={styles.fieldContainer}>
                ...
                <View style={styles.inputContainer}>
                  <Icon name="counter" size={20} color="#95a5a6" style={styles.inputIcon} />
                  <Controller
                    control={control}
              ...
              <View style={styles.fieldContainer}>
                ...
                <View style={styles.inputContainer}>
                  <Icon name="note-text" size={20} color="#95a5a6" style={styles.inputIcon} />
                  <Controller
                    control={control}
